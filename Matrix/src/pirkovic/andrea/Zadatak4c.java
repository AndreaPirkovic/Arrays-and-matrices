...
public class Zadatak4c {
	public static void main(String[] args) {
		...
		System.out.print("Unestite skalar: ");
		skalar = sc.nextInt();
		for (int i=0; i<red; i++) {
			for (int j=0; j<red; j++) {
			c[i][j]= skalar * (a[i][j]+b[i][j]);
			}
			}
		System.out.println("Elementi dvodimenzionalnog niza c pomnozeni skalarom su:");
		for (int i=0; i<red; i++) {
			for (int j=0; j<red; j++) {
