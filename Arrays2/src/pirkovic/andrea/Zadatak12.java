...
public class Zadatak12 { public static void main(String[] args) throws Exception { 
	...
	int n = Integer.parseInt(ulaz. readLine()); // Unos elemenata niza x 
	...
	for (int i = 1; i <= n; i++) { 
		...
		int k = 1; 
		double s = (Math.abs(Math.sin(x[i] / k) - Math.pow(Math.cos(x[i]), 2))) / (k * k + 1); while (Math.abs(s) <= eps) { 
			k++; 
