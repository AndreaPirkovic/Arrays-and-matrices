...
public class Zadatak17 { public static void main(String[] args) throws Exception { 
	...
	for (int i = 1; i <= brojRadnika; i++) { 
		...
		ime[i] = ulaz.readLine(); System.out.print("Broj bodova: "); 
		brojBodova[i] = Double.parseDouble(ulaz.readLine()); System.out.print("Rata kredita: ");
kredit[i] = Double.parseDouble(ulaz.readLine()); System.out.print("Članarina: "); 
...
suma += licniDohodak[i]; } 
	...
	// Štampanje rezultata 
	for (int i = 1; i <= brojRadnika; i++) 
		System.out.println(ime[i] + "\t" + brojBodova[i] + "\t" + kredit[i] + "\t" + clanarina[i] + "\t" + licniDohodak[i]); System.out.println("==================================================="); System.out.println("Ukupno za isplatu: \t\t\t" + suma); } }
